...
impl Transpiler for RustTranspiler {
    ...
    fn transpile_component_def<'a>(
        ...
        let mut params: Vec<Doc<'a>> = component
            .parameters
            .iter()
            .map(|param| {
                arena
        ...
        if component.has_slot {
            self.needs_slot = true;
            params.push(arena.text("slot: &Slot"));
        }
    ...
    fn transpile_component_call_statement<'a>(
        ...
        let mut all_args: Vec<Doc<'a>> = args
            .iter()
            .map(|arg| {
                arena
                    .text("&")
        ...
        if !slot_body.is_empty() {
            // Block expression that shadows output, wrapped in Slot
            self.needs_slot = true;
            let children_doc = arena
                .text("&Slot({ let mut output = String::new(); ")
                .append(self.transpile_statements(arena, slot_body))
                .append(arena.hardline())
                .append(arena.text("output })"));
            all_args.push(children_doc);
        }
...
mod tests {
    ...
    fn component_with_children() {
        ...
        let expected = expect![[r#"
            ...
            fn render_wrapper(slot: &Slot) -> String {
                let mut output = String::new();
                output.push_str("<div>");
            ...
            impl View for Main {
                fn render(self) -> String {
                    let mut output = String::new();
                    output.push_str(&render_wrapper(&Slot({ let mut output = String::new(); output.push_str("<p>Hello</p>");
                    output })));
                    output
                }
            }
