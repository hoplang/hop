use crate::common::{
    Token, TokenType, Node, NodeType, Range, Position, RangeError, is_void_element
};
...
impl NodeBuilder {
    ...
    fn append_child(&mut self, parent_id: usize, node: Node) -> usize {
        ...
        self.children.entry(parent_id).or_insert_with(Vec::new).push(idx);
        ...
        match token.token_type {
            ...
            TokenType::EndTag => {
                if is_void_element(&token.value) {
                    errors.push(RangeError::new(
                        format!("Tag <{}> should not be closed using a closing tag", token.value),
                        token.range,
                    ));
                    ...
                    if !found_match {
                        errors.push(RangeError::new(
                            format!("Expected closing tag for <{}>", 
