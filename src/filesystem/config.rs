...
pub struct CompileConfig {
    /// The language to compile to: "ts", "js", "go", or "python"
...
impl HopConfig {
    ...
    pub fn get_resolved_config(&self) -> ResolvedConfig {
        ...
        ResolvedConfig {
            target,
            output_path,
            backend_commands: self.compile.backend_commands.clone(),
            pages: self.compile.pages.clone(),
            go_package,
        }
    }
}
...
mod tests {
    ...
    fn get_resolved_config() {
        ...
        assert!(resolved.backend_commands.is_empty());
        assert_eq!(resolved.pages, vec!["main/App"]);
    ...
    fn parse_config_with_backend_commands() {
        let toml_str = indoc! {r#"
            ...
            output_path = "app.ts"
            pages = ["main/App"]
            backend_commands = ["npm install", "npm start"]
        "#};
        let config = HopConfig::from_toml_str(toml_str).unwrap();
        let resolved = config.get_resolved_config();
        assert_eq!(resolved.target, TargetLanguage::Typescript);
    ...
    fn parse_config_without_backend_commands() {
        ...
        "#};
        let config = HopConfig::from_toml_str(toml_str).unwrap();
        assert_eq!(config.compile.target, Some(TargetLanguage::Javascript));
        assert!(config.compile.backend_commands.is_empty());
    }
    ...
    fn full_target_names_rejected() {
        ...
        // "javascript" should be rejected (only "js" is valid)
        let toml_str = indoc! {r#"
            [compile]
            target = "javascript"
            output_path = "app.js"
