    ...
    fn transpile_module(&mut self, module: &IrModule) -> String {
        ...
        if self.needs_trusted_html {
            let trusted_html = BoxDoc::nil()
                ...
                .append(BoxDoc::line())
                .append(BoxDoc::line())
                .append(BoxDoc::text("/** Marks a string as trusted HTML, bypassing escaping. Only use with sanitized or trusted content. Calling this function with untrusted content causes XSS vulnerabilities. */"))
                .append(BoxDoc::line())
                .append(BoxDoc::text("export function trustHtml(str: string): TrustedHTML {"))
                .append(
                    BoxDoc::nil()
                        .append(BoxDoc::line())
                        .append(BoxDoc::text("return str as TrustedHTML;"))
                        .append(BoxDoc::line())
                        .nest(4),
                )
                .append(BoxDoc::text("}"))
