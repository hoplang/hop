...
pub struct HopConfig {
    ...
    /// Build configuration (optional - only needed for `hop build`)
    #[serde(default)]
...
pub struct BuildConfig {
    /// The language to build to: "ts", "go", "python", or "rust"
...
pub enum ConfigError {
    #[error(
        "Missing 'build.target' in hop.toml. Expected one of: \"ts\", \"go\", \"python\", \"rust\""
    )]
    MissingTarget,
    #[error("Missing 'build.output_path' in hop.toml")]
    MissingOutputPath,
}
...
impl HopConfig {
    ...
    /// Returns an error if `build.target` or `build.output_path` are missing.
    pub fn get_resolved_config(&self) -> Result<ResolvedConfig, ConfigError> {
        let target = self.build.target.ok_or(ConfigError::MissingTarget)?;
...
mod tests {
    ...
    fn should_accept_config_with_typescript_target() {
        ...
        assert_eq!(config.build.target, Some(TargetLanguage::Typescript));
        assert_eq!(config.build.output_path, Some("app.ts".to_string()));
    }
    ...
    fn should_accept_all_valid_target_values() {
        ...
        "#};
        let config = HopConfig::from_toml_str(toml_str).unwrap();
        assert_eq!(config.build.target, Some(TargetLanguage::Go));
        ...
        "#};
        let config = HopConfig::from_toml_str(toml_str).unwrap();
        assert_eq!(config.build.target, Some(TargetLanguage::Python));
    }
    ...
    fn get_resolved_config_should_error_on_missing_target() {
        ...
        assert!(err_msg.contains("build.target"));
    ...
    fn get_resolved_config_should_error_on_missing_output_path() {
        ...
        assert!(result.is_err());
        let err_msg = result.unwrap_err().to_string();
        assert!(err_msg.contains("build.output_path"));
        assert!(err_msg.contains("hop.toml"));
    }
